...
const uint64_t pipeOut = 0xE8E8F0F0E2LL; // Gửi tín hiệu
const uint64_t pipeIn =  0xE8E8F0F0E4LL; // Nhận phản hồi

RF24 radio(9, 10);// CE=9, CSN=10
const int button1Pin = 2;// Nút nhấn Giường 1
const int button2Pin = 3;// Nút nhấn Giường 2
const int ledPin = 4;   // Led phản hồi Phòng 2
...
void setup() {
  ...
  pinMode(ledPin, OUTPUT);           // Chân LED là đầu ra

  // Cấu hình module RF24
  radio.begin();                      // Bắt đầu RF24
...
void loop() {
  ...
  if (digitalRead(button1Pin) == LOW) {
    sendSignal(3); // Gửi tín hiệu "3"
  }

  // Gửi tín hiệu khi nút 2 được nhấn
  if (digitalRead(button2Pin) == LOW) {
    sendSignal(4); // Gửi tín hiệu "4"
  }
}
